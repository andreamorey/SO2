                   ...
                   unsigned int *p_entrada, char reservar, unsigned char permisos)
{
    ...
    { // la entrada no existe
        ...
        case 1:
            if (inodo_dir.tipo == 'f')
            { // no podemos crear entradas dentro de un fichero
                return ERROR_NO_SE_PUEDE_CREAR_ENTRADA_EN_UN_FICHERO;
            }
            if ((inodo_dir.permisos & 2) != 2)
            { // si no tienen permiso de escritura
            ...
            else
            {
                ...
                { // directorio
                    if (strcmp(final, "/") == 0)
                    { // reservamos inodo tipo directorio
                    ...
                    { // no existe el directorio intermedio
                        //fprintf(stderr)
                ...
                { //entramos si ha habido fallo
                    if (entrada.ninodo != -1)
                    { //si la entrada ya tenía un ninodo, lo liberamos porq ya
...
int mi_dir(const char *camino, char *buffer){
    ...
    if (inodo.tipo == 'd') {
        if (camino[strlen(camino)-1] != '/'){
            fprintf(stderr, RED"La sintaxis es incorrecta falta '/'\n"RESET);
            return -1;
        }
        ...
        struct entrada entrada;
        mi_read_f(ninodo_dir, &entrada, nentrada * sizeof(struct entrada), sizeof(struct entrada));

        mi_dir_aux(entrada, buffer);
        entradasLeidas++; 
...
void mi_dir_aux(struct entrada entrada, char *buffer){
    struct inodo inodo;
...
int mi_write(const char *camino, const void *buf, unsigned int offset, unsigned int nbytes)
{
    ...
    //fprintf(stderr, "Bytes escritos de mi_write(): %d\n", bytesEscritos);
...
int mi_link(const char *camino1, const char *camino2)
{
    ...
    //Libermaos el inodo que se ha asociado a la entrada creada
    liberar_inodo(p_inodo2);

    //incrementamos la cantidad de enlaces de p_inodo1 y actualizamos el ctime
    inodo.nlinks++;
...
int mi_unlink(const char *camino){
    ...
    struct entrada entrada;
    leer_inodo(p_inodo_dir, &inodo_dir);
    int cantEntradas = inodo_dir.tamEnBytesLog / sizeof(struct entrada);
    struct entrada entradaEliminar;
    // leemos la entrada a eliminar para saber su inodo
    mi_read_f(p_inodo_dir, &entradaEliminar,(p_entrada)*sizeof(struct entrada), sizeof(struct entrada));

    if (p_entrada != cantEntradas-1){
        //miramos la última entrada y escribimos sobre la entrada que queremos eliminar
        mi_read_f(p_inodo_dir, &entrada,(cantEntradas-1)*sizeof(struct entrada), sizeof(struct entrada));
    ...
    //eliminamos: 
    leer_inodo(entradaEliminar.ninodo, &inodo);
    inodo.nlinks = inodo.nlinks- 1; 
    if( inodo.nlinks == 0){
        liberar_inodo(entradaEliminar.ninodo); 
    }else{
        inodo.ctime = time(NULL);
        escribir_inodo(entradaEliminar.ninodo, inodo);
    }
    return 0;
}