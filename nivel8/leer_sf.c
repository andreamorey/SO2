...
int main(int argc, char**argv){
    if (argc != 2){   // Validación de sintaxis
        fprintf(stderr, "sinatxis: ./leer_sf <nombre_dispositivo>\n");
        return 0;
    }
    char* nombre_dispositivo = argv[1];
    bmount(nombre_dispositivo);  // abrimos el fichero
    struct superbloque SB;
    if(bread(POSSB,&SB) == -1){  // leemos los datos del superbloque
        fprintf(stderr, "No se ha podido leer el Superbloque\n");
        return -1;
    }
    // visualizamos los datos del superbloque:
