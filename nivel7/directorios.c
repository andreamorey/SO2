...
char *final, char *tipo){
    ...
    while((camino[x] != '/') && (camino[x] != 0)){  // copiamos el primer fichero
...
unsigned int *p_entrada, char reservar, unsigned char permisos){
    ...
    // aqui hay unos mallocs (ade)
    char final[strlen(camino_parcial)];

    struct entrada buffer_entradas[BLOCKSIZE/sizeof(struct entrada)]; // para la mejora
    ...
    // unsigned int inodo_dir = *p_inodo_dir
    ...
    //fprintf(stderr, "[buscar_entrada() -> cant_entradas_inodo: %d", cant_entradas_inodo);
    ...
    if(cant_entradas_inodo > 0 ){
        ...
        leer_inodo(*p_inodo_dir, &inodo_dir);
        buffer_entradas[0].ninodo = inodo_dir.
    }
    */

    int num_entrada_buffer = num_entrada_inodo % cant_entradas_inodo;
    ...
    if(mi_read_f(*p_inodo_dir, buffer_entradas, num_entrada_inodo * sizeof(struct entrada),
        ...
        fprintf(stderr, "Error: directorios.c -> buscar_entrada()\n");
        return -1;
    }
    // prints
    ...
    while((num_entrada_inodo < cant_entradas_inodo) && strcmp(entrada.nombre,inicial) != 0){
        num_entrada_inodo++;
        num_entrada_buffer = num_entrada_inodo % cant_entradas_inodo;
    ...
    if((num_entrada_inodo == cant_entradas_inodo) && (strcmp(entrada.nombre, inicial) != 0)){ // la entrada no existe
        switch(reservar){
            ...
            case 1:
                ...
                if ((inodo_dir.permisos & 2) != 2){ // si no tienen periso de escritura
